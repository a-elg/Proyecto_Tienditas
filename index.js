...
app.post("/signup", async (request, response) => {
  ...
  if (
    ...
    data.registred = await MySQL.createCustomer(
      request.body.email,
      request.body.name,
      request.body.tel,
      request.body.password
    ...
    if (data.registred)
      data.u_name = await MySQL.readCustomer(request.body.email).c_name;
  }
  response.send(data);
  response.end();
});
...
app.post("/signin", async (request, response) => {
  ...
  if (request.body.email == "" || request.body.password == "") {
    ...
    let signin = await MySQL.signinCustomer(
      request.body.email,
      request.body.password
    );
...
app.post("/ssignin", async (request, response) => {
  ...
  if (request.body.email == "" || request.body.password == "") {
    ...
    let signin = await MySQL.signinStore(
      request.body.email,
      request.body.password
    );
    data.u_name = signin.name;
...
app.post("/dmsignin", async (request, response) => {
  ...
  if (request.body.email == "" || request.body.password == "") {
    ...
    let signin = await MySQL.signinDelivery(
      request.body.email,
      request.body.password
    );
    data.u_name = signin.name;
...
app.post("/catalog", async (request, response) => {
  ...
  if (
    request.headers.referer.includes(
      `http://${process.env.domain}/home.html`
    ) ||
    request.headers.referer.includes("http://127.0.0.1:5000/home.html")
  ) {
    console.log(request.body);
...
app.get("/test", async (req, res) => {
  ...
  console.log("->");
  let Test_res;
  await MySQL.readDeliveryMan("erana17@yopmail.com").then((res) => {
    Test_res = res;
